...
export default function Studio() {
  ...
  async function downloadCards() {
    const sheet = document.querySelector<HTMLElement>(".photo-card");
    ...
    try {
      ...
      const canvas = await html2canvas(sheet, {
        ...
        windowWidth: 1280,
        onclone: async (doc) => {
          // Capture the same fixed A6 card used in the preview.
          const clonedSheet = doc.querySelector<HTMLElement>(".photo-card")!;
      ...
      pdf.save("allan-shiphira-a6-photo-card.pdf");
    } catch {
  ...
  return <main className="studio">
    ...
    <div className="studio-workspace">
      ...
      <button className="studio-print" disabled={!qr || downloading} onClick={downloadCards}>{downloading ? "Creating PDF…" : "Download A6 card as PDF ↓"}</button>
      ...
      <p className="studio-help">Downloads one A6 card (105 × 148 mm) per PDF page, with your black-and-white portraits and stamped QR code. Print at actual size (100%). Enable contributions in your album so guests can add photos.</p><p className="studio-help">Unlisted card-making tool. No sign-in is configured.</p>
      </aside>
      <section className="studio-preview" aria-label="Printable photo cards"><div className="studio-preview-label"><span>YOUR TABLE CARDS</span><span>A6 / 105 × 148 MM</span></div><div className="studio-sheet">
      {[0].map(n => <div className="studio-cut" key={n}><article className="photo-card"><h2>KARIBU SANA</h2>
